...
def parse_json_to_data(json_data):
    ...
    # Split SkillSets into technical & architectural (basic heuristic)
    skills_list = [s.strip() for s in profile["SkillSets"].replace("TOOLS :", ",").split(",")]
    technical_skills = [{"name": s, "level": 80} for s in skills_list if s]
    ...
    return {
        ...
        "title": f"Software Developer | {profile['YearsOfExp']} years experience",
        "contact": {
            "phone": profile["MobileNo"],
            "email": profile["EmailId"],
            "location": "India"
        },
        "photo": None,  # You can pass a file path here if available
        "summary": (f"Experienced software developer with {profile['YearsOfExp']} years "
                    f"of expertise in Java, microservices, and cloud technologies. "
                    f"Strong background in system design, enterprise applications, and "
                    f"cloud migrations."),
        "architectural_skills": [
            {"name": "System Design", "level": 90},
            {"name": "Microservices", "level": 95},
            {"name": "Cloud Migration", "level": 90}
        ],
        "technical_skills": technical_skills[:8],  # take first 8 skills
...
with open("/Users/priyankapc/PycharmProjects/TestScript/jsons/soumik.json", "r") as f:  # put your JSON file name here
    json_data = json.load(f)

data = parse_json_to_data(json_data)
generate_creative_resume(data, f"{data['name'].replace(' ', '_')}_architech_negative_CV.pdf")
